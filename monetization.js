(function () {
  ...
  async function showAndroidBannerAd() {
    ...
    if (!isConfigured(ANDROID_BANNER_AD_UNIT_ID)) {
  ...
  async function prepareInterstitialAd() {
    ...
    if (!isConfigured(ANDROID_INTERSTITIAL_AD_UNIT_ID)) return false;
  ...
  function maybeShowInterstitialAfterPageViews() {
    if (!isAndroidCapacitor() || !isConfigured(ANDROID_INTERSTITIAL_AD_UNIT_ID)) return;
